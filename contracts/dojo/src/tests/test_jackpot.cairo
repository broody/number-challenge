...
mod tests {
    ...
    use nums::{
        systems::{
            ...
            jackpot_actions::{
                jackpot_actions, IJackpotActionsDispatcher, IJackpotActionsDispatcherTrait
            }
        },
        models::{
            game::m_Game, slot::m_Slot, config::{m_Config, Config, GameConfig},
            jackpot::jackpot::{m_Jackpot, Jackpot}, jackpot::mode::JackpotMode,
        }
    };
    ...
    fn KING_OF_THE_HILL_STATE(
        ...
        let mut jackpot_actions = IJackpotActionsDispatcher {
            contract_address: jackpot_actions_address
        };
        ...
        let jackpot_id = jackpot_actions
    ...
    fn test_create_king_of_the_hill() {
        let (world, jackpot_id, mut game_actions, mut jackpot_actions) = KING_OF_THE_HILL_STATE(
        ...
        assert(
            jackpot.expiration == EXPIRATION_TIME + (EXTENSION_TIME * 2), 'expiration extends 400'
        );
        assert(king_of_the_hill.king == PLAYER_ONE(), 'player should be king');
    ...
    fn test_king_me_multiple() {
        let (_, jackpot_id, mut game_actions, mut jackpot_actions) = KING_OF_THE_HILL_STATE(
            START_BLOCK_TIME, EXPIRATION_TIME, EXTENSION_TIME
        );
